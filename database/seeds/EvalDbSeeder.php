    ...
    public function run()
    {
      ...
      factory(SET\User::class, 3)->create(['role' => '', 'status' => 'separated',
        ...
        foreach ($createdUsers as $createdUser) {
            if ($createdUser->destroyed_date) { continue; } // Do not assign user
            if ($createdUser->id < 3) { continue; } // Exclude admin
            foreach ($createdGroups as $createdGroup) {
                if ( rand(1,6)!=1 ) { continue; } // So not all users get group
        ...
        foreach ($createdTrainings as $createdTraining) {
            if ( $createdUser->id < 3 ) { continue; } // Exclude admin
            if ( rand(1,4)==1 ) { continue; } // So not all trainings get types
            ...
            if (++$ttCount > $createdTrainingTypes->count()) {  $ttCount = 1; }
        }
        ...
        foreach ($createdDuties as $duty) {
            ...
            foreach ($createdUsers as $createdUser) {
                    if ( $createdUser->id < 3 ) { continue; } // Exclude admin
                    if ($createdUser->destroyed_date) { continue; } // do not assign
                    if ( rand(1,4)!=1 ) { continue; } // So not all get one
        ...
        for ($i=0; $i < 3; $i++) {
            ...
            $groupIDs = [];
            foreach ($createdGroups as $createdGroup) {
                if ( rand(1,2)!=1 ) { continue; } // So not all get one
        ...
        foreach ($createdUsers as $createdUser) {
            if ($createdUser->destroyed_date) { continue; } // do not assign
            if ( $createdUser->id < 3 ) { continue; }
            for ($i=0; $i < 6; $i++) {
                if ( rand(1,20)!=1 ) { continue; } // So not all get one
        ...
        foreach ($createdUsers as $createdUser) {
            if ( $createdUser->id < 3 ) { continue; }
            if ( rand(1,6)==1 ) { continue; } // So not all get one
